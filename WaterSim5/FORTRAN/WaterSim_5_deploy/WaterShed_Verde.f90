      ...
      subroutine monthlyBC(T,i,sumDemand,Ain)
        ...
        ! ====================
            ...
            !
             ...
             !
             Ain%gvf_classAVerdeToSaltMonthly_acft(i)=0
            !
            ! Reduce B and C allocations to Verde if the estimate storage falls below 45 % of Maximum
            ! 11.05.14 DAS
            ! ------------
            if(1 < i)then
              if(mvf_stateAdjustDemand(T%year,i-1) < lpf_threshStorage)then
               mvf_tempClassBCrights_acft =  mvf_tempClassBCrights_acft * (1.-mvf_stateAdjustDemand(T%year,i-1))
              endif
            else
              if(2000 < T%year)then
                if(mvf_stateAdjustDemand(T%year-1,12) < lpf_threshStorage)then
            ...
            if(mvf_ClassADeficitMonthly(i) < 0)then
              Ain%gvf_verdeToSaltTransferClassA_acft  = Ain%gvf_verdeToSaltTransferClassA_acft+ABS(mvf_ClassADeficitMonthly(i))
              Ain%gvf_classAVerdeToSaltMonthly_acft(i)= abs(mvf_ClassADeficitMonthly(i))
              !
...
!            real(8), parameter :: lpd_exponent=2.71828182845904523536
            ...
            ! ===================
                ...
                do j = 1,lvi_loop,1
                    ...
                    lvf_reservoirToBCwater= (1.-lvf_ratioVerde) * mvf_differenceBCVerdeMonthly(1,i)
                    !
                     lvf_differenceBC=0
                    ...
                    !
                      ...
                      if(lvf_ratioVerde < (1.-lpf_rounding))lvl_rounding=.true.
                    !
                    if(mpf_storageCapacityVerde_acft <= annual)then
                        !
                        ! Increate the amount of water removed from storage for B&C
                        ! -------------
                        lvf_ratioVerde=(max(0.,lvf_ratioVerde*lpf_deltaStorage))
                        !
                        ...
                        ! ---------------------------------------------------------------------------------------------
                        !
                        lvf_storageThreshold=mpf_storageMinimumVerde*lpf_proportionalSaveVerdeStorage*cf
                        !
                    ...
                    if(lvl_pass)then
                     exit
                        ...
                        ! -------------------------------------------------------------------
                        !
                        lvf_ratioVerde=min(1.,lvf_ratioVerde*lpf_deltaStorage)
                        !
                    endif
                    !
                end do
                    ...
                    Ain%gvf_verdeToSaltTransferClassBC_acft   = Ain%gvf_verdeToSaltTransferClassBC_acft+anint(mvf_differenceBCVerdeMonthly(2,i))
        ...
        subroutine initializeLoop(T,Ain)
            ...
            !
                ...
                Ain%gvf_verdeToSaltTransferClassA_acft=0
                Ain%gvf_verdeToSaltTransferClassBC_acft=0
        ...
        subroutine outputsToInterface(T,Ain)
            ...
            !
                ...
                go_VerdeClassAused_acft=nint(Ain%gvf_verdeAused_acft)
                 go_verdeRiverFlow_acft=0
                go_verdeRiverFlow_acft=nint(mvf_flowVerde_acft*gvf_ratioVerdeFlowsPostEnv)
