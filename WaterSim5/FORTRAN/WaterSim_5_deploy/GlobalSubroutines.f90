...
! 
    ...
    subroutine openFiles(module_,lvc_DPath_,file_,LU_)
      use gm_GlobalData
        ...
        character(len=200) :: lvc_dataPath='App_Data\WaterSim_5_0\'
        logical :: checkFID
        ! ===========================================================
        !
            !
...
!            rate(Uprovider)=min(maxRate,gvi_AgPumpCurveIndex)
    ...
    !          call sPopulationMask(T,lvl_capTrue,lvf_creditWeight)
            ...
            !
            ! Set as 2015 (gvi_baseYear currently 2016)
...
10 format(I4,1x,4(F9.1,1x))
           ...
           ! gvf_AgCreditWeight(T%year,Uprovider)=lvf_creditWeight(Uprovider)
            !
             lvf_diffT    =0
             lvf_adjustPumping=1.0735 ! Parameterized to bring GW to (near) zero by 2085 when WEBAGTR1 eq 100%
                                      ! 02.28.17
            if(T%year .lt. gvi_baseYear-1)then
            ...
            ! Time step differences in credits used - estimated curve
                ...
                ! ---------------------------------------------------------------------------------------
                 !lvf_diffT  = min(gvf_diff, lvf_adjustPumping * max(0,lvf_benchMarkCredits- (gvf_AgPumpingEst_acft_a(2,Uprovider,k)+ gvf_EffluentToUrban(T%year)*lvf_creditWeight(Uprovider)) ))
                 lvf_diffT  = lvf_adjustPumping * max(0,lvf_benchMarkCredits- (gvf_AgPumpingEst_acft_a(2,Uprovider,k)+ gvf_EffluentToUrban(T%year)*lvf_creditWeight(Uprovider)))
                else
            ...
            if(T%year .lt. gvi_baseYear+1)then
             ...
             !
!write(103,*)T%year, go_AgWaterGW_AF(Uprovider),lvf_diffT,lvf_creditWeight(Uprovider),lvf_fallowProportion
...
!                   lvf_creditTransCAP(Uprovider)=0
        ...
        if(0 < lvf_creditWeight(Uprovider))then
           ...
           ! endif
              ...
              if(T%year .gt. 2002)then
                do i = 1,7,1
                  do j = 1,2,1
                    ...
                    !
                    surface_Odefault(i)=gvf_AgSurfOtherEstDefault_acft_a(j,k-i)
               ...
               do i = 1,7,1
                  do j = 1,2,1
                    ...
                    surface_Odefault(i)=gvf_AgSurfOtherEstDefault_acft_a(j,k)
            ...
            !
            gvf_AgSurfOtherEstDefault_acft_a(1,k)=gv_AgWaterPumpingSRV(T%year,2)
            gvf_AgSurfOtherEstDefault_acft_a(2,k)=agAMAavailableDefault(1)
            !
...
subroutine AgAcerageAndCredits(T,lvf_relArea)
 use lms_LULC
    ...
    real :: lvf_totAcreage
    real :: lvf_relArea(gvi_maxProV)
    real,parameter :: lpf_minCredits=0.01
    ...
    ! ============================
        ...
        if(gvl_LULCstatus)then
            ...
            if(T%year < 2011)then
                ...
                !
 1000 format(I4,1x,I2,1x,I4,1x,3(F8.3,1x))
                !
            endif
...
1003 format(I4,1x,L2,1x,I2,1x,4(F10.2,1x))
        ...
        do i = 1,gvi_Providers,1
          lvf_relArea(i)=0
            gvf_AgCreditWeight(T%year,i)=0
            if(0 < lvf_totAcreage)then
             if(0 < lvf_totAcreage)then
               lvf_relArea(i)=max(lpf_minCredits,gvf_acerageAg(T%year,i) / lvf_totAcreage)
             endif
             ! 11.20.16
