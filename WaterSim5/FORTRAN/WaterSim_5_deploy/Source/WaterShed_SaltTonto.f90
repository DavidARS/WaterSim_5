  ...
  use gm_GlobalData
   use gm_TypeControl
    ...
    !
        ...
        real, private :: mvf_flowControl,mvf_flowControlHold,mvf_reservoirRelease
        !
...
!      end subroutine releaseSalt
    ...
    subroutine storageSaltMonthly(T,i,sumDemand,Ain)
        ...
        !
        real :: lvf_criteria
        real :: lvf_upperThreshold, lvf_UpperOther
        real :: cf,lvf_addFromRoosevelt
        real :: lvf_modCriteria
        ...
        ! ===================
            ...
            !
            lvf_UpperOther=mpf_storageCapacityOther_acft*lvf_upperThreshold
            lvf_criteria=0
            !
            do j = 1,n,1
                !
                ...
                ! ---------------------------------------
                if(mvl_rooseveltMin .or. mvl_rooseveltMax)lvf_modLowerThreshold=1.
                ! Pre-process
                ! --------------------------------------------
                !  01.26.17
                mvf_flowControlHold=mvf_flowControl
                !
                if(mpf_storageCapacityOther_acft < annual)then
                    ...
                    if(mpf_storageMinimumOther_acft < annual)then
                        ...
                        !*cf was added on 01.25.17 BUT WAS IN the debug version.....
...
!      end subroutine transferOut
      ...
      subroutine outputsToInterface(T,Ain)
            ...
            ! ========================
                ...
                go_saltTontoRiverFlow_acft= nint(mvf_flowSaltTonto_acft*gvf_ratioSaltTontoFlowsPostEnv)
                !
       ...
       use gm_ModelControl
        ...
        ! ==================
            ...
            call agRetirement(T%simyear,af)
             ...
             lvf_constAgDemand= max(0,lvf_responseSTVerde*lpf_Ag)
             !
            ...
            if(0.25 < lvf_ratioStorage)lvf_addFlowToAgRatio=lvf_ratioStorage
            !
             lvf_demandFromAg=0
            lvf_demandFromAg=lvf_constAgDemand +lvf_addFlowToAgRatio*lvf_flowAnnual
            !
              TotDemand=0
             TotDemand= Aout%gvf_verdeToSaltTransferClassA_acft
            if(0 < TotDemand)then
            !
1           continue 
...
2           continue
            ...
            !
             TotDemand= Aout%gvf_verdeToSaltTransferClassBC_acft
            if(0 < TotDemand)then
