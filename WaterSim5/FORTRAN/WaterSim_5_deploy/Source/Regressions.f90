 ...
 use gm_GlobalData
  use gm_Exception
    ...
    1   continue
        ...
        do j = 1,iterations,1
          lvf_sum=0
          lvf_adjust=(lvf_total-lvf_min)*lvd_mod
        ! +23
10      do k = 1,12,1
            lvf_out(k)=0
...
subroutine Nmodifier(it,lvf_mag,lvf_sum,lvf_tot,lvd_mod)
    ...
    !
      !
      lvd_diffPct=lvf_sum/lvf_tot
      !
      if(lvd_diffPct <= 0.95)then
          !
           if(lvf_sum < lvf_tot)then
              lvd_mod=lvd_mod*1/lvp_eightTens
           else
              lvd_mod=lvd_mod*lvp_eightTens
           endif
           !
       ...
       !
          if(0.999 < lvd_diffPct .and. lvd_diffPct < = 1.001)then
            if(lvf_sum < lvf_tot)then
              lvd_mod=lvd_mod*1/0.99999
            else
              lvd_mod=lvd_mod*0.99999
            endif
          else if(0.99 < lvd_diffPct .and. lvd_diffPct < = 1.01)then
            if(lvf_sum < lvf_tot)then
              !
              if(10000 < lvf_mag)then
               lvd_mod=lvd_mod*1/lvp_TenThous
              else
               lvd_mod=lvd_mod*1/lvp_Thous
                if(500 < it )lvd_mod=lvd_mod*1/lvp_TenThous
              endif
              !
            else
              if(10000 < lvf_mag)then
                lvd_mod=lvd_mod*lvp_TenThous
              else
                lvd_mod=lvd_mod*lvp_Thous
                if(500 < it )lvd_mod=lvd_mod*1/lvp_TenThous            
              endif
            endif
           ...
           !
            ...
            if(lvf_sum < lvf_tot)then
              ...
              lvd_mod=lvd_mod*1/lvp_Hundredths
            else
              lvd_mod=lvd_mod*lvp_Hundredths
            endif
          endif
       !
       !
          ...
          !
           if(lvf_sum < lvf_tot)then
              lvd_mod=lvd_mod*1/lvp_Tenths
           else
              lvd_mod=lvd_mod*lvp_Tenths
           endif
           !
       else
         !
