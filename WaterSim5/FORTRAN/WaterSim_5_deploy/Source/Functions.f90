...
Module lms_ClimateFactor
 ...
 Contains
    ...
    function fClimateFactorCO(i,gvi_StartF,year,lvi_CF)
        ...
        ! =======================================
        !
            ! lvi_CF as a percentage of the change in runoff (more intuitive?)
            ! That is why I am doing all the decimal manipulations. 08.06.12
    !        if(i < 2)then     
            fClimateFactorCO=1.
            !
            if(gvl_start)then
            !if(i == gpi_startCF)then
             call ClimateParms(lvi_CF,year,m,b)
               ...
               gvf_LinearClimateFactorCO(3)=lvi_CF*0.01
                ...
                !
                if(gvf_LinearClimateFactorCO(3) < lvf_equationBase)then
                    fClimateFactorCO= lvf_equationBase
                else
                    fClimateFactorCO=gvf_LinearClimateFactorCO(3)
                endif
            !
      return
    ...
    !        if(i < 2)then  
            ...
            lvf_equationBase=fLinear(m,x,b)
            !
            if(gvf_LinearClimateFactorSVT(3) < lvf_equationBase)then
                fClimateFactorSVT= lvf_equationBase
            else
                fClimateFactorSVT=gvf_LinearClimateFactorSVT(3)
            endif
            !
        !
      return
...
!
  ...
  ! --------------------------
   ...
   ! ---------------------------
!    function fRoundToValue(variable,places)   
!        !
!        ! ------ Types -------
!        integer :: Int
!        real :: variable
...
! -------------------------------------------------------
    ...
    ! -------------------------------------------------------
    function fLogistic(Time,a,years) result(temp)
        ! --------- types ----------
        integer :: Time
        real :: a,b,c  
        real :: years,temp
