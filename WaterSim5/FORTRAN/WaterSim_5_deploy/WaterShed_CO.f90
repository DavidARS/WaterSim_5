...
!               Infile='App_Data\Data\CORiver_BofRbaseUpdate.txt'; LU=10
...
!            integer, parameter :: lpi_CObaseRecords=16
   ...
   use gm_TypeControl
    use gm_Euler
      ...
      !
            ...
            !
            real, parameter :: mpf_pan_mead=2.28092  ! 89.8 inches average (http://pubs.usgs.gov/sir/2006/5252/pdf/sir20065252.pdf)
            real, parameter :: mpf_pan_reach=4.5
            real, parameter :: mpf_pan_powell=1.763776  ! 69.44 inches average (adjusted pan)(http://www.riversimulator.org/Resources/Hydrology/EvaporationAtLakePowellUCRCopt.pdf)
            real, parameter :: mpf_mod_BankStorage=1.5
            !
   ...
   contains
    ! ======
      ...
      subroutine aFlow_b(T,acft)
        ...
        integer :: stop=16 !integer :: stop=13 ! use 13 for 2000 through 2012
...
!                call aOutToInterface(T)
            ...
            if(.not. gpl_comparisons)then
                ...
                ! 06.26.13
                ! 
                 lvf_powellToMeadInFlowsTyearMinusOne(T%jumpYear)=0
                lvf_powellToMeadInFlowsTyearMinusOne(T%jumpYear)=lvf_inflows*cf 
                 !
            ...
            ! 
10 format(I4,1x,3(F11.5,1x),I4,1x,I4)
        return
      ...
      subroutine aModifyFlow_bb(T)
        !
        ! -------------- Types ----------------
        real :: maxEnvCO,AfterFlowToEnvironment
        integer :: gvi_WaterToEnvironCO_acft_a
        ! =====================================
        !
      ...
      subroutine expectedStorage_ba(T,wsb)
        ...
        subroutine expectedStorage_bb(T,wsb)
            ...
            !
                ...
                ! ------------------------------------------------------------------------
                  lv_Evap_bb=0.
                lv_Evap_bb=fEvaporation_Mead(vState_Bb_maf(T%year),mpf_pan_mead)
                !
                  lvf_flux=0
        ...
        function fBankStoragePowell(lvf_diffStorage)
            ...
            !
                !
                fBankStoragePowell=lvf_diffStorage*lpf_bankStoragePowell
                !
            !
         return
        ...
        function fBankStorageMead(lvf_diffStorage)
            ...
            ! ==========================================
                !
                fBankStorageMead=lvf_diffStorage*lpf_bankStorageMead
                !
            !
         return
            ...
            ! ========================
               !
                ...
                if(gvl_AZdeltaBurden)then
                  ...
                  lvf_requestCOdeltaWater_nv=gvi_WaterToCOdelta_acft_a*(vNallocation_nv/(lvf_totalCOallocatedLB+(mvf_totalCOallocatedUB*gpd_acftTomaf)))
                  lvf_requestCOdeltaWater_ca=gvi_WaterToCOdelta_acft_a*(vNallocation_ca/(lvf_totalCOallocatedLB+(mvf_totalCOallocatedUB*gpd_acftTomaf)))
                  lvf_requestCOdeltaWater_mx=gvi_WaterToCOdelta_acft_a*(vNallocation_mx/(lvf_totalCOallocatedLB+(mvf_totalCOallocatedUB*gpd_acftTomaf)))
                endif
                !
                 lvf_temp=0
                if(T%year < 2003)then
                ...
                if(T%startyear < T%year)then
                 lvf_diffInFlow=mvf_powellToMeadInflows_maf- lvf_powellToMeadInFlowsTyearMinusOne(T%simyear)
                endif
                !
...
!write(107,2)T%year,wsb%lv_BaToBb(T%year), lv_Mead_Add_Flow,lv_Evap_bb,lv_allocation_normal,lvf_fluxPreBank
