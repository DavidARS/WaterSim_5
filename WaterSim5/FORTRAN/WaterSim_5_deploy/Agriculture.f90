...
!                !
                ...
                !if( 0 < go_AgEfficiency)lvf_AgEfficiency=go_AgEfficiency*1/100
...
30 format(I4,1x,4(F10.1,1x))
                    ...
                    !
                    ! gv_AgWaterPumpingSRV(T%year,6) is the user defined pumping by Ag (or, credit transfer difference)
                     GW=gv_AgWaterPumpingSRV(T%year,6) !* lvf_AgEfficiency**0.93
                     SW= max(0, min(gvf_ratioMaricopaAg*lvf_agPool,gv_AgWaterPumpingSRV(T%year,5)))
                     !
                      gvf_AgWaterTotals(1)=GW
                      gvf_AgWaterTotals(2)=SW
                    ...
                    !
!                    lvf_temp=go_pumpingAgTotal_AF(T%year)
if(2015 < T%year)then
!write(103,1003)T%year,go_AgProductionPCT,lvf_AgEfficiency,GW,SW,effluent, lvf_temp,GW,lvf_unPumpedGW
endif
10   format(I4,1x,I4)
100  format(I4,1x,5(F12.2,1x))
1000 format(I4,1x,7(F9.0,1x))
1003 format(I4,1x,I4,1x,7(F10.2,2x))
