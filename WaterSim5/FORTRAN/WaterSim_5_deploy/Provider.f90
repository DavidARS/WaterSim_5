...
100         continue
        ...
        subroutine sProvider(T,vGP,vLP)
            ...
            ! ======================
                ...
                ! vPV%lvf_cDemand_Res_outdoor_acft_annual(i)
                ! vPV%lvf_cDemand_Res_indoor_acft_annual(i)
                ! vPV%lvf_cDemand_Ind_outdoor_acft_annual(i)
                ! vPV%lvf_cDemand_Ind_indoor_acft_annual(i)
                ! --------------------------------------------------------------
        ...
        ! ----------------------------------
         subroutine aDemandSupply(T,vGP,vLP)
            ...
            ! ---------------------------------- Types ----------------------------------------------------------------
             ...
             integer :: lvi_One,lvi_Two,lvi_Three,lvi_Four,lvi_Five,lvi_Six,lvi_Seven,lvi_Eight,lvi_Nine,lvi_Ten
             integer :: lvi_Eleven,lvi_Twelve,lvi_Thirteen,lvi_Fourteen
...
!                call sCalculateAddedDemand(T)
            temp_add=0
              ...
              if(gpl_DemandDriven)then
                  ...
                  !
                 lvi_One=0; lvi_Two=0; lvi_Three=0; lvi_Four=0; lvi_Five=0; lvi_Six=0; lvi_Seven=0
                 lvi_Eight=0; lvi_Nine=0; lvi_Ten=0; lvi_Eleven=0; lvi_Twelve=0; lvi_Thirteen=0; lvi_Fourteen=0
                  ! 
                      ...
                      if(gpl_writeProviderDotTxt)then
                        if(gvl_APIcleared)then
                          ...
                          do j = 1,gpi_unusedNonPotable,1
                           lvi_One=nint(gvf_WaterDemand_acft(i,j,1))
                           lvi_Two=nint(gvf_WaterDemand_acft(i,j,2))
                           lvi_Three=nint(gvf_WaterDemand_acft(i,j,3))
                           lvi_Four=nint(gvf_WaterDemand_acft(i,j,4))
                           lvi_Five=nint(gvf_WaterDemand_acft(i,j,5))
                           !
                           lvi_Six=nint(gvf_WaterDemand_acft(i,j,6))
                           !
                           lvi_Seven=nint(gvf_WaterDemand_acft(i,j,7))
                           lvi_Eight=nint(gvf_WaterDemand_acft(i,j,8))
                           lvi_Nine=nint(gvf_WaterDemand_acft(i,j,9))
                           lvi_Ten=nint(gvf_WaterDemand_acft(i,j,10))
                           !
                           lvi_Eleven=nint(gvf_WaterDemand_acft(i,j,11))
                           lvi_Twelve=nint(gvf_WaterDemand_acft(i,j,12))
                           lvi_Thirteen=nint(gvf_WaterDemand_acft(i,j,13))
                           lvi_Fourteen=nint(gvf_WaterDemand_acft(i,j,14))
!                           lvi_Fifteen=nint(gvf_WaterDemand_acft(i,j,15))
!                           lvi_Sixteen=nint(gvf_WaterDemand_acft(i,j,16))
!                           lvi_Seventeen=nint(gvf_WaterDemand_acft(i,j,17))
!                           lvi_Eighteen=nint(gvf_WaterDemand_acft(i,j,18))
                           !
                           ! Write to Provider.txt
                           !  
                           write(4,5)T%year,i,j,lvi_One,lvi_Two,lvi_Three,lvi_Four,lvi_Five,lvi_Six,lvi_Seven, &
                                lvi_Eight,lvi_Nine,lvi_Ten,lvi_Eleven,lvi_Twelve,lvi_Thirteen,lvi_Fourteen
...
!                      else if(nint(gvf_WaterDemand_acft(i,j,6)) == gpi_waterOther)then
                        ...
                        !
                        gii_regReclaimed=gii_regReclaimed+vLP%lvf_reclaimed(i)
                        !
                  ...
                  do i = 1,gvi_Providers,1
                    ...
                    ! -------------------------------------------------------------------------
                      ...
                      ! ----------------------------------------------------------------------------------------------
                        ...
                        ! 02.21.13
                        ! ---------------------------------------------------
                         vGP%lvf_newWaterSupplies_acft_annual(i)=0.
                        vGP%lvf_newWaterSupplies_acft_annual(i)=gvf_newSuppliesUsed_acft_a(i) 
                        !
...
!                    go_annualDeliveriesCO(k)   =NINT(vGP%gRealizedDesignationCAP_acft(T%year,k))     ! acft a-1
                    ...
                    !
temp_add=temp_add+go_annualDeliveriesCO(k)
...
write(1,*)"provider.f90 line 731",T%year,temp_add
          !
5           format(I4,1x,2(I2,1x),14(I8,1x),I2)
           return
...
!                        vGP%lvf_rainWaterHarvested(i)= vGP%lvf_outdoorNonPotable(i)+vGP%lvf_indoorNonPotable(i)
                        !
                       else if(nint(gvf_WaterDemand_acft(i,j,6)) == gpi_stormWaterCapture)then
                        !
...
!                        vGP%lvf_stormWaterCaptured(i)=vGP%lvf_outdoorNonPotable(i)+ vGP%lvf_indoorNonPotable(i)
                        !                      
                       else if(nint(gvf_WaterDemand_acft(i,j,6)) == gpi_grayWater)then
                        !
                        vGP%lvf_outdoorNonPotable(i)= vGP%lvf_outdoorNonPotable(i) &
...
!              E%TStep=1
              ...
              do i = 1,gvi_Providers,1
               ...
               ! -----------------------------------------------------------------
                    vGP%lvf_cDemand_Res_outdoor_acft_annual(i)=anint(gvf_TotalDemand_acft(i) &
                     * gvf_parm_WStoRes_prop(i)*gvf_parm_OutDoorResProp(i))

                    vGP%lvf_cDemand_Res_indoor_acft_annual(i) =anint(gvf_TotalDemand_acft(i) &
                     * gvf_parm_WStoRes_prop(i)*(1-gvf_parm_OutDoorResProp(i)))

                    vGP%lvf_cDemand_Ind_outdoor_acft_annual(i)=anint(gvf_TotalDemand_acft(i) &
                     * gvf_parm_WStoInd_prop(i)*gvf_parm_OutDoorIndProp(i))

                    vGP%lvf_cDemand_Ind_indoor_acft_annual(i) =anint(gvf_TotalDemand_acft(i) &
                     * gvf_parm_WStoInd_prop(i)*(1-gvf_parm_OutDoorIndProp(i)))
                    !
100 format(I4,1x,I2,1x,1(F11.2,1x))

                    ! 08.08.12 Ajay & Johnston Project
                    ! --------------------------------------------------------------------------------
                ! - 04.01.13
                    if(gvl_modelVinzeJohnston)then
                        vGP%lvf_cDemand_Com_outdoor_acft_annual(i)=anint(gvf_TotalDemand_acft(i) &
                         * gvf_parm_WStoCom_prop(i)*gvf_WaterToCommercialTurf_Prop)

                        vGP%lvf_cDemand_Com_indoor_acft_annual(i) =anint(gvf_TotalDemand_acft(i) &
