...
!               gvf_newSuppliesUsed_acft_a(i), then, is passed to city_model via  vGP%lvf_newWaterSupplies_acft_annual(i)
...
!               influences demand for class A water. The variable is gvi_NormalFlowEmpiricalMax_acftac(i)
!           06.09.14
!             19) Cleaned up code
!           07.18.14
!            19) Cleaned up code
...
!
!       version 5.8.1 Added a return statement to the parameter control subroutine for reading default parameters
!                   removed redundancy in reading the land cover land use data
!
!       version 5.8.2
!           27 July, 2016 - 1) removed two variables from the model: go_rightsLostP4(i),go_rightsLostP5(i).  I decided that
!           they held no meaning.....
!                           2) 
!       version 5.8.3
!           25 August, 2016 - 1) Serious bug in the Designations_CO.f90 file for  gvf_AgCAPpotential_AF(i).
!             it was NOT being re-set annually, so the  go_AgWaterUsedByMuni_PCT_a(i) outputs were
!             greviously incorrect (FUCK)
!
!       version 5.8.4
!           29 August, 2016 - 1) I completely changed the method to estimate the PCT Ag water transfer to Urban
!           see Provider.f90 starting on line 764. The output variable is go_AgWaterUsedByMuni_PCT_a(i)
!
!       version 5.8.5
!           31 August, 2016 - 1) I completely changed the Ag sustainability metric. I moved go_AgWaterUsedByMuni_PCT_a(i)
!             to Agriculture.f90, I added code to estimate groundwater and surface water use by Ag, and subtract
!             water transfers. This new indicator will be a "production" indicator for Ag.
!             2) I redefined  go_AgWaterCAPavailable_AF(j) in Designations_CO.f90 to represent CAP water used by Ag, 
!             3) I changed the "rights" percentages for Ag transfers to represent the ratio of Ag lands to total lands
!                AND NOT population-based as it was before. This may be found in GlobalSubroutines in the subroutine
!                labeled  AgAcerageAndCredits(lvf_relArea). It is called from the subroutine sPopulationMask
!             4) Added a global variable for: a) gvf_acerageTotal(j), b) gvf_acerageAg(i)
!
!      version 5.9.2
!           4 September 1)  gvf_acerageAg(i) NOW  gvf_acerageAg(T%year,i)
!           6 October: 1) fixed the function call fVariableFactorX() and, 2) fClimateFactorCO(i,gvi_StartF,year,lvi_CF)
!               to add the gvi_StartF parameter (is was missing; not clear how the complier never caught it before)
!
!      version 5.9.3
!           18 October, 2016
!               1) I changed the ag indicator data to be agriculture production (see Agriculture.f90)subroutine 
!                   AgricultureProduction(T,effluent)
!
!      versions 5.9.4 through 5.9.6 apparently were NOT documented
!
!      version 5.9.7
!           26 January, 2017
...
!       version 5.9.8
!           8 February, 2017
!               11) Set go_AgEfficiency to equal One
! ---------------------------------------------------------------------------------------------------------------------
!    
