...
!
!      open(90,FILE=trim(trim(lvc_dataPath)//'App_Data\Data\agPumpingSurface_2085.txt'),status='old')
     ...
     ! ---------------------------
      ! ------------------------
        subroutine readFiles_pc()
            ...
            !
!           read(80,*,err=800,iostat=ios, FMT="(7(F5.2,1x)")gdf_parm_WWtoRWWTP(j),gdf_parm_RWWtoRO(j),gdf_parm_WWtoEFF(j), &
!                                           gdf_parm_EFFtoVadose(j),gdf_parm_EFFtoPP(j),gdf_parm_SWtoWBamt(j), &
!                                           gdf_parm_OutDoorResPCT(j)
            !
  ...
  ! ---------------------------
    ...
    subroutine initSVT(i,lvi_countSRP)
        ...
        if(gvl_maskSRP(i))then
         ...
         gvi_NormalFlowEmpiricalMax_acftac(lvi_countSRP)=90
           !
    ...
    subroutine initPopAndDemand(i)
        ...
        go_CAPunusedPriority5(i)=0
             ...
             gvf_population=0
             gvi_ReductionDemand_pctOfBaseline=1
...
!            go_ReclaimedDifference_acft(i)=0
            ...
            ! 06.21.16
            do j = 1,10,1
             gdf_parms_newValueRetain(i,k)=0
            end do
            !
            !
        return
    ...
    subroutine initGeneral(i)
        ...
        ! =================
            ...
            !
            gvi_ProviderAgCreditCurveIndex(i)=10
            ! ---------------------------------
        ...
        ! =============            
            ...
            do j = 1,gvi_Providers,1
              ...
              !
               ...
               go_AgWaterGW_AF(j)=0
               go_AgWaterGW_PCT(j)=0
            end do
            !
            do i = 1,100,1
              do j = 1,2,1
                gvf_AgSurfOtherEst_acft_a(j,i)=0
                gvf_AgSurfOtherEstDefault_acft_a(j,i)=0
...
!
             ...
             !
             !gvf_agDemand_acft(i)=0
             !gvf_agUnusedEffluent(i)=0
             !gvf_agUsedEffluent(i)=0
             ! 06.20.16
...
!
!    ! --------------------------
!    subroutine initializeParmControl()
!      use lm_ParameterControl
!        !
!        call initParmControl()
!        !
!       return
!     end subroutine initializeParmControl
!    ! -----------------------------
 ...
 contains
    ...
    subroutine modifyDesignations(T)
        ...
        ! ---------------------
            ...
            lvi_max=gvi_NormalFlowEmpiricalMax_acftac
            !
            do j = 1,10,1
               lvf_out(j)=0.9 
            end do
...
!
 ...
 subroutine pCityParameterControl_a(T)
   use lms_ParameterControl
        ...
        real :: lvf_compliance,span
        real :: lvf_YearsToInflection
        real :: fLogistic
        !logical :: lvl_catchAPI(gvi_maxProV)
        ! ================
        !
            ...
            if(T%year .EQ. T%startyear)then
             ...
             !
              ...
              !   call invokeDefault(i)
                ...
                !
if(i == 8 .or. i == 25)then
!write(1,120)T%year,i,gdf_parm_WWtoRWWTP(i),gvf_parm_WWtoRWWTP(i),gvf_parm_RWWtoRO(i),gdf_parm_RWWtoRO(i), &
!                    gvf_parm_WWtoEffluent(i),gdf_parm_WWtoEFF(i),gvf_parm_EffluentToVadose(i),gdf_parm_EFFtoVadose(i),  &
!                    gvf_parm_EffluentToPP(i),gdf_parm_EFFtoPP(i),gvf_parm_SWtoWBamount(i),gdf_parm_SWtoWBamt(i),&
!                    gvf_parm_OutDoorResProp(i),gdf_parm_OutDoorResPCT(i)*0.01
endif
...
!                gvf_parm_WWtoRWWTP(i)=0.
 ...
 subroutine checkDefault(i,T)
   ...
   integer :: i
    ...
    if(anint(1000*gdf_parms_newValueRetain(i,3))== anint(1000*gdf_parm_WWtoEFF(i)))then
      gvl_defaultRun(3,i)=.true.   
    else
      gvl_defaultRun(3,i)=.false.
    endif
    if(anint(1000*gdf_parms_newValueRetain(i,4))==  anint(1000*gdf_parm_EFFtoVadose(i)))then
      gvl_defaultRun(4,i)=.true.   
    else
      gvl_defaultRun(4,i)=.false.
    endif
    if(anint(1000*gdf_parms_newValueRetain(i,5))== anint(1000*gdf_parm_EFFtoPP(i)))then
      gvl_defaultRun(5,i)=.true.   
    else
      gvl_defaultRun(5,i)=.false.
    endif
    if(anint(1000*gdf_parms_newValueRetain(i,6))== anint(1000*gdf_parm_SWtoWBamt(i)))then
      gvl_defaultRun(6,i)=.true.   
    else
      gvl_defaultRun(6,i)=.false.
    endif
    if(anint(1000*gdf_parms_newValueRetain(i,7))== anint(1000*gdf_parm_OutDoorResPCT(i)*0.01))then
