...
5           format(I4,2x,I2,2x,1(F9.0,1x))
       ...
       !-------------------------
        ...
        subroutine reductionsInDesignations(T,vDb,gvi_order,lv_capMandI,lvf_capNIA)
            ...
            real:: lvf_maxP4(gvi_maxProV),lvf_maxP5(gvi_maxProV)
            real:: lvf_availableP4,lvf_availableP5
            real:: lvf_desired_IV(gvi_maxProV),lvf_desired_V(gvi_maxProV)
            ...
            real:: lvf_AgPool
            real:: lvf_reduceAg,lvf_reduceAgMax
            !
...
10 format(I4,1x,2(F9.1,1x),F11.1)
                ...
                do i = 1,gvi_Providers,1
                    call sAgSurfaceWaterOther(T,i,lvf_addSurfWaterFromAgCAP)
                     !gvf_AgCAPpotential_AF(i)= gvf_AgCAPpotential_AF(i)+ lvf_addSurfWaterFromAgCAP(i)
...
!            vReductionIV=1.
            ...
            !
            !lvf_capPfive=lvf_capNIA-lvf_capPsix
...
!                    lvf_ratioSIX=lvf_creditsSIX/lvf_totalSIX
                ...
                if(0 < gvi_agCAPcreditsTransfered(j))then
                    lvf_usedP6(j)=(min(gvi_agCAPcreditsTransfered(j)*lvf_reduceAg,lvf_AgPool*lvf_ratioAgAdditions(j)))
...
!                    lvf_ratioSIX=lvf_creditsSIX/lvf_totalSIX
                    ...
                    if(0 < lvf_capPsix)then
                     lvf_capPfive=lvf_capPfive+(min(gvi_agCAPcreditsTransfered(j)*lvf_reduceAg,lvf_AgPool*lvf_ratioAgAdditions(j)))
                    !
                    ...
                    ! Will I use this?
                    ! 04.15.16
!                         lvf_difference_V(j)=(lpf_NIA_Priority-sum(lvf_desired_V))*lvf_ratioTotalDesig_V(j)
                      endif        
                    endif
...
!1003          format(I4,1x,I2,1x,4(F12.3,1x)) !format(I4,1x,I2,1x,3(F(10.4,1x)))
            do i = 1,gvi_Providers,1
             ...
             ! lvf_CAPneeded_IV(i)=lvf_CAP_needed(i) * lvf_ratioDesignation_IV(i)
             !
             ! NEW on 07.29.15 das
!              lvf_maxP4(i)=vDb%lvf_Designation_IV_B_acft(T%year,i)
!              lvf_maxP5(i)=vDb%lvf_Designation_V_B_acft(T%year,i)
!             !
!             ! NEW on 08.13.15
!               lvf_desired_IV(i)= min(lvf_maxP4(i),lvf_CAP_needed(i) * lvf_ratioDesignation_IV(i))
!               lvf_desired_V(i) = min(lvf_maxP5(i),lvf_CAP_needed(i) * (1-lvf_ratioDesignation_IV(i)))
                ...
                !go_rightsLostP4(i)=nint(max(0,lvf_maxP4(i)-lvf_available_IV(i)))
                !
               ...
               ! lvf_reduceFive=0
                ...
                if(lvf_capPfive <=lvf_addDesignation_V)then
                    ...
                    if(0 < lvf_addDesignation_V)then
                      if(lvf_capPfive <=lvf_addDesignation_V)then
                      ! lvf_reduceFive=lvf_capPfive/lvf_addDesignation_V
                      endif
                    endif
                    !
                    lvf_available_V(i)=lvf_capPfive*lvf_ratioTotalDesig_V(i)     
            ...
            do i = 1,gvi_Providers,1
                vDb%lv_realizedDesignation_B_acft(T%year,i)=lvf_usedP4(i)+lvf_usedP5(i)
            end do
            !
             lvf_availableP4=0
             lvf_availableP5=0
